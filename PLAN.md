...
- [x] 테스트 페이지 추가
- [ ] 메인페이지 Explore 섹션 상위로 이동
- [ ] RSS 관련 내용 제거
- [ ] 현재 페이지의 상위 페이지로 돌아가는 버튼 추가 (예: /docs/java/class.md → /docs/java/)
  - 공식 기능이나 모듈을 사용하는 것을 우선적으로 고려, 없다면 진행 하기 전에 논의
- [ ] 제목 두 번 나오는 문제 수정(frontmatter title과 본문 H1 중복)
- [ ] 제목의 `` 기호 적용 되지 않는 문제 수정 (예: `Java 17` → Java 17로 수정하거나 or ` ` 문법 적용되도록 수정)
- [ ] mermaid 다이어그램 렌더링 문제 수정 (단순 코드블록으로 표시되는 문제)
- [ ] 기술 블로그 컨텐츠 조회 시 왼쪽 사이드바 보이지 않도록 변경
- [ ] DOCS도 블로그처럼 작성일 / 수정일 / 작성자 정보가 보이도록 반영(현재 blog와 동일한 방식으로 적용)
- [ ] DOCS 메인 페이지 NOT FOUND 문제 수정
- [ ] DOCS 메인 페이지 들어갔을 때 대분류가 나열되어 그 안에 문서가 존재하는 트리 형태 디자인으로 하기(기존의 docs/README.md 형태 참고)
- [ ] 기술 블로그 포스팅(blog) 메인 페이지 접속 시 최근 포스팅과 함께 내용이 조금씩(한 두 줄) 보이는 형태로 디자인(현재는 포스팅이 연속으로 나열되는 형태로 보임)
