...
export const SCENARIOS = [
    ...
    {name:"PG 5xx — 재시도로 복구",outcome:{status:"DONE",color:"done",props:["재시도로 복구 (4회 미만)","3중 중복 방지"]},impact:"\n· 재시도가 없다면 가장 빈번한 장애인 네트워크 순단마다 유입 결제가 그대로 실패\n· 짧은 순단이 매출 손실로 이어지는 경로 차단",hops:[
      ...
      {edge:"pg~self",kind:"retry",label:"self-loop 재발행 · 간격 늘림 · 횟수 +1",why:{tag:"왜 self-loop",text:"시도 횟수를 DB(pg_inbox.attempt)에 기록해 한도(4회)까지 관리한다. 메시지 헤더에 두면 유실될 수 있어 DB 한 곳에만 저장했다.",dref:5}},
    ...
    {name:"EOS abort — DLQ 재주입",outcome:{status:"DONE",color:"done",props:["관리자 재주입으로 복구","멱등 재처리"]},impact:"\n· 메시지가 유실되면 벤더 승인은 완료됐지만 주문은 미완으로 남음\n·건별 확인·환불 대응이 필요한 정산 불일치 발생\n· DLQ 보존과 원 토픽 재주입으로 전량 복구 보장",hops:[
      ...
      {edge:"dlq>payment",kind:"recover",label:"reprocess-dlq · 원 토픽 재발행",why:{tag:"왜 원 토픽",text:"별도 자동 컨슈머 대신 검증된 EOS 파이프라인을 재사용한다. 이미 상태가 종결되었거나 발행 후 8일이 경과한 메시지는 타임스탬프 검증으로 차단한다.",dref:4}},
    ...
    {name:"EOS 재전송 — 재고 확정 보상 재발행",outcome:{status:"DONE",color:"done",props:["상태 재전이 차단","재고 확정만 보상 재발행"]},impact:"\n· 재고 확정 이벤트가 유실된 채 종결되면 결제는 완료인데 재고는 미확정으로 잔류\n· 재전송을 신호 삼아 누락 이벤트만 다시 발행해 결제·재고 정합 회복",hops:[
      {edge:"pg>payment",kind:"normal",label:"events.confirmed APPROVED 수신",state:"→ DONE"},
      {edge:"payment~self",kind:"fail",label:"Kafka 커밋 유실 (stock-committed 미발행)"},
      {edge:"payment~self",kind:"retry",label:"events.confirmed 재전송 (오프셋 미커밋)"},
      {edge:"payment~self",kind:"recover",label:"종결 가드 · 전이 스킵 (이미 DONE)",state:"DONE 유지",why:{tag:"왜 재발행",text:"종결 가드는 상태 재전이만 막는다. DB는 DONE인데 재고 확정 이벤트만 유실된 재전송이면, 상태는 그대로 두고 그 이벤트만 다시 발행한다.",dref:6}},
    ...
    {name:"격리 — 관리자 안전 종결",outcome:{status:"FAILED",color:"failed",props:["안전 종결","유령 재고 0","감사 기록"]},impact:"\n· 격리 건을 임의로 성공 처리하면 입금 없는 유령 매출, 맹목 복원하면 유령 재고 발생\n· 실패 종결 단일 경로와 조건부 복원이 두 손실 모두 차단",hops:[
      ...
      {edge:"payment>redis",kind:"recover",label:"차감 표식이 있을 때만 재고 복원",why:{tag:"왜 조건부",text:"차감 표식(decrement:done)이 '실제로 차감했다'는 사실의 원본 기록이다. 차감한 적 없는 결제까지 복원하면 유령 재고가 발생한다 — 표식이 있을 때만 복원해 이를 막는다.",dref:3}},
      {edge:"payment~self",kind:"recover",label:"조건부 갱신: status=QUARANTINED → FAILED (order 동조)",state:"QUARANTINED → FAILED",why:{tag:"왜 FAILED만",text:"격리 진입은 벤더 승인 전이라 실제 결제가 발생하지 않았다. DONE으로 복구하면 유령 매출 — 안전 실패 종결만 지원한다.",dref:2}}
    ]},
    ...
    {name:"재고 차감 후 저장 실패 — 자동 복구 배제",outcome:{status:"FAILED",color:"failed",props:["초과 판매 0","미복구 수량 지표화"]},impact:"\n· 저장 실패 후 자동 롤백을 시도하다 크래시하면 차감만 남고 복구 기록은 유실\n· 자동 복구를 포기하고 미복구 수량을 지표로 노출 — 초과 판매 없이 수동 회수",hops:[
      ...
      {edge:"payment~self",kind:"fail",label:"DB 확정 저장 실패 (자동 롤백 배제)",why:{tag:"왜 자동 복구 안 함",text:"자동 롤백 도중 크래시하면 차감 기록만 남고 복구 내역은 유실돼 추적 불가한 불일치가 된다. 자동 복구 대신 미복구 수량을 지표로 노출해 수동 회수한다.",dref:0}}
    ]},
    ...
    {name:"종결 가드 — 늦은 승인 무시",outcome:{status:"QUARANTINED",color:"quar",props:["늦은 승인 폐기","상태 불변 보장"]},impact:"\n· 종결 뒤 도착한 승인이 상태를 덮어쓰면 격리 판정이 뒤집혀 정합성 붕괴\n· 종결 가드가 늦게 온 메시지를 폐기해 종결 결과 보존",hops:[
      {edge:"pg>payment",kind:"warn",label:"events.confirmed APPROVED 메시지를 늦게 수신"},
      {edge:"payment~self",kind:"recover",label:"종결 가드 · 전이 무시 (guard skip)",state:"QUARANTINED 유지",why:{tag:"왜 무시",text:"결제가 이미 종결 상태면 이후 도착한 전이 시도를 무시한다 — 늦은 승인이 종결 결과를 덮어쓰지 못하게 막는다."}}
    ]},
...
export const SCN_OBS = [
    ...
    ["payment_confirm_guard_skip_total"],
...
export const SCN_MATRIX = [
    ...
    ["격리 관리자 종결","failed","토큰 조건부 보상","관리자 안전 종결 (조건부 갱신)","transition QUARANTINED→FAILED"],
    ["재고 부족","failed","미차감","즉시 실패 (409)","transition to=FAILED"],
    ["재고 차감 후 저장 실패","failed","미복구 (지표화)","자동 복구 배제","stock_retention_unrecovered"],
    ["재고 동시 결제 원자 차감","failed","미차감 (밀린 건)","즉시 실패 (원자 직렬화)","transition to=FAILED"],
    ...
    ["종결 가드 늦은 승인","quar","없음","전이 무시 (guard skip)","confirm_guard_skip"],
