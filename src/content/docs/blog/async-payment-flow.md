...
%% 클래스 정의 (가독성 및 테마 대응 규칙 적용)
    ...
    subgraph SyncPath ["confirm 핵심 경로 — 단일 TX"]
        direction TB
        A["POST /confirm"]:::external --> B["PaymentEvent: IN_PROGRESS"]:::main
        B --> C["PaymentOutbox: PENDING 저장"]:::pending
        C --> D["ApplicationEvent 발행"]:::main
        D --> E["202 Accepted 즉시 반환"]:::note
    end

    subgraph Polling ["클라이언트 상태 폴링"]
        direction TB
        P1["GET /payments 상태 조회"]:::external --> P2{"결제 상태 확인"}:::pending
        P2 -- " IN_PROGRESS " --> P3["일정 시간 대기"]:::note
        P3 -.->|재조회| P1
        P2 -- " DONE " --> P4["최종 결제 완료 처리"]:::success
    end

    subgraph Background ["백그라운드 처리 (Outbox Strategy)"]
        direction TB
        F["AFTER_COMMIT 이벤트 수신"]:::main --> G["Toss API 호출"]:::external
        G --> H["PaymentEvent: DONE"]:::success
        H --> I["PaymentOutbox: DONE"]:::success
    %% 폴백 및 복구 로직
        J["OutboxWorker @Scheduled 폴백"]:::recovery --> K["PENDING 배치 조회"]:::pending
        K --> G
    end
...
%% 클래스 정의 (가독성 및 장애 단계 구분)
    ...
    classDef external fill: #F5F5F5, stroke: #333, color: #000
    classDef recovery fill: #FADAD8, stroke: #B85450, color: #000
    A["confirm 요청"]:::external --> B["ImmediateHandler 동시 실행"]:::main
    B --> C["각 핸들러: DB 커넥션 점유"]:::main
%% 장애 가속 구간
    C --> D["HikariCP 한계 도달<br/>(Pool Exhaustion)"]:::pending
    D --> E["신규 confirm도 DB 커넥션 대기"]:::pending
%% 최종 장애 상태
    E --> F["에러율 80%, TPS ≈ 0<br/>(System Failure)"]:::recovery
%% 위험 흐름 강조
...
%% 클래스 정의 (역할별 색상 구분 및 가독성 확보)
    ...
    subgraph SyncPath ["HTTP 경로 — 빠른 반환 (Non-blocking Entry)"]
        direction TB
        A["POST /confirm"]:::external --> B["confirm() @Transactional"]:::http
        B --> C["PaymentOutbox PENDING 저장"]:::queue
        C --> D["ApplicationEvent 발행"]:::http
        D --> E["OutboxImmediateEventHandler<br/>(AFTER_COMMIT)"]:::http
        E --> F["channel.offer(orderId)"]:::http
        F --> G["202 Accepted (즉시)"]:::response
    end

    subgraph Channel ["PaymentConfirmChannel (In-Memory Buffer)"]
        H["LinkedBlockingQueue<br/>(capacity=5000)"]:::queue
        F --> H
    end

    subgraph WorkerPath ["Worker 경로 — 비동기 처리 (Virtual Threads)"]
        direction TB
        I["OutboxImmediateWorker<br/>(SmartLifecycle)"]:::worker
        I --> J["Worker VT-1: queue.take()"]:::worker
        I --> K["Worker VT-2: queue.take()"]:::worker
        I --> L["Worker VT-N: queue.take()"]:::worker
        J --> M["OutboxProcessingService.process()"]:::worker
        K --> M
        L --> M
        M --> N["Toss API 호출 + DB markDone()"]:::external
    end

    subgraph FallbackPath ["폴백 경로 (Batch Recovery)"]
        O["OutboxWorker<br/>@Scheduled(fixedDelay=3s)"]:::fallback
        O --> P["PENDING 배치 조회 → process()"]:::queue
        P --> M
    end

%% 컴포넌트 간 상호작용
    G -->|" 202 응답 후 폴링 시작 "| CP1["GET /payments 상태 조회"]:::external
    H --> J
    H --> K
    H --> L
...
%% 클래스 정의 (가독성 및 테마 대응 규칙 적용)
    ...
    classDef success fill: #E8F5E9, stroke: #2E7D32, color: #000
    A["confirm 요청"]:::entry --> B["DB: PENDING 저장"]:::data
    B --> C{"channel.offer"}:::data
%% Fast Track: 실시간성 보장
    C -- " true " --> D["Fast Track<br/>(Worker VT 즉시 처리)"]:::fast
%% Safety Track: 안정성 보장 (Backpressure)
    C -- " false (오버플로우) " --> E["PENDING 유지"]:::safety
    E --> F["Safety Track<br/>(OutboxWorker 폴링 3s)"]:::safety
%% 장애 복구 트리거
    H["서버 재시작"]:::entry --> F
%% 최종 결과
    D --> G["결제 완료"]:::success
