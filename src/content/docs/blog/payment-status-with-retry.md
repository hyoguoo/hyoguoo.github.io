...
description: "모든 결제 에러를 실패로만 처리하던 문제를 결제 상태 전환 테이블과 재시도 로직으로 개선하여 복구 가능한 에러를 처리하는 시스템을 구축한다."
---
...
이를 해결하기 위해 [대규모 시스템 설계 기초 — 결제 시스템](/docs/large-scale-system/system-design-interview/design-payment-system/)
파트의 재시도 방식을 참고해 시스템을 재설계하여, 이를 적용해보았다.
