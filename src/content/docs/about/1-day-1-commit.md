...
1일 1커밋과 [solved.ac 알고리즘 풀이](https://solved.ac) 스트릭을 유지한지 많은 시간이 흘러 1,000+일을 넘긴지 오래다.(26/05/07 기준, 종종 갱신 중..)   
사실 이제는 어떠한 장/단점을 떠나 단순 재미로 유지하고 있긴 하지만, 처음 시작했던 계기와 장/단점 그리고 후기를 적어보려고 한다.

++ 현재는 BOJ가 서비스 종료되어 LeetCode에서 풀이를 계속 유지 중이다.
...
> ++ 사실 예전에도 자동화를 통해 유지하려면 할 수 있었고, 현재는 특히나 AI를 통해 진행하게 되면 더더욱 의미가 없어진 지표가 됐다.  
> 하지만 근본은 이를 유지하기 위한 마인드셋이라고 생각하며, 나는 스스로 이를 약속 혹은 시스템이라 생각하며 계속해서 지속해나갈 것 같다.
