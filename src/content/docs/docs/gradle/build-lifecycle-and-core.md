...
특정 버전의 Gradle을 로컬에 별도 설치하지 않고도 빌드 환경을 동일하게 유지하기 위한 핵심 도구이다.

- 일관성: 팀원 모두가 동일한 Gradle 버전을 사용하도록 강제하여 "내 컴퓨터에서는 되는데" 문제를 방지
- 구성 요소: 실행 스크립트(`gradlew`), 설정 파일(`gradle-wrapper.properties`), 실제 구동 로직이 담긴 `wrapper jar`로 구성
- 재현성: 새로운 환경(CI/CD 서버 등)에서 추가 설정 없이 `./gradlew` 명령만으로 빌드 시작 가능
