...
flowchart LR
    ...
    Mem_A -- 스왑 아웃 --> Disk_A_Image
    Disk_B -- 스왑 인 --> Mem_Space
...
| 최적 적합 (Best Fit)  | 모든 빈 공간을 검색하여 프로세스 크기에 가장 딱 맞는(가장 작은) 공간에 할당 |      외부 단편화는 줄지만, 아주 작은 빈 공간(파편)을 많이 생성       |
| 최악 적합 (Worst Fit) |          모든 빈 공간을 검색하여 가장 큰 공간에 할당           | 작은 파편 생성은 줄지만, 큰 빈 공간을 빠르게 소진하여 큰 요청을 받기 어려워짐 |
