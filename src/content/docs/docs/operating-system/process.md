...
description: "프로세스의 메모리 영역 구조와 문맥 교환 원리, PCB의 역할, 프로세스 상태 변화, fork/exec 시스템 호출과 Copy-On-Write 메커니즘을 정리한다."
---
...
`fork()`는 부모의 모든 자원을 자식에게 상속하지만, 호출 시점에 메모리 페이지를 전부 복사하지는 않는다.

|       시점       |              메모리 상태               |   추가 RAM 사용량    |
|:--------------:|:---------------------------------:|:---------------:|
|    fork 직후     |     부모·자식이 같은 물리 페이지를 매핑만 공유      |  0 (페이지 테이블만)   |
| 한쪽이 페이지에 쓰기 발생 | 해당 페이지(보통 4KB)만 복제하여 양쪽이 각자 사본 보유 | 수정된 페이지 수 × 4KB |
|     쓰기 누적      |         사본 페이지가 점진적으로 증가          |   누적 수정량에 비례    |

```mermaid
graph TD
    Fork([fork 호출]) --> Share["부모와 자식이 동일 물리 페이지에 매핑"]
    Share --> Decision{어느 한쪽이 페이지에 쓰기 발생}
    Decision -->|아니오| KeepShared["페이지 공유 유지 - 추가 RAM 0"]
    Decision -->|예| Copy["해당 페이지만 4KB 단위로 복제"]
    Copy --> Diverge["부모와 자식이 각자 다른 페이지를 사용"]
    Diverge --> Accumulate["쓰기 누적 시 사본 페이지도 누적"]
```

결과적으로 fork의 비용은 전체 메모리 복사가 아니라 페이지 테이블 설정 + 수정되는 페이지에 한해 점진적 복사가 된다.

- 사본은 쓰기를 일으킨 쪽에 새로 할당되고, 다른 쪽은 원본 페이지를 그대로 유지
- `fork()` 직후 곧바로 `exec()`로 메모리를 덮어쓰는 자식의 경우 모든 페이지가 어차피 버려지므로, COW가 없다면 통째로 복사한 결과가 그대로 폐기되는 낭비 발생
