...
description: "운영체제의 핵심인 커널의 역할과 이중 모드 동작 원리를 설명하고 시스템 호출·시그널 처리 흐름을 분석한다."
---
...
개별 시스템 호출의 Mode Switch 비용은 작지만, 호출 빈도가 누적되면 응답 시간과 처리량에 영향을 준다.

- 매 호출의 Mode Switch가 CPU 파이프라인과 캐시·TLB의 일부를 무효화
- 빈도 높은 환경(로그 출력, 네트워크 송수신 등)에서는 사용자 공간 버퍼링으로 호출 횟수 자체를 감소 (예: `BufferedWriter`, Logback `immediateFlush=false`)
- 또는 비동기 패턴으로 워커 스레드에 I/O를 위임해 호출 스레드의 비용 부담 자체를 제거

## 시그널(Signal) 처리

시그널은 커널이 프로세스에 비동기적으로 이벤트를 통보하는 메커니즘이다.
...
- 기본 동작: 핸들러를 등록하지 않으면 OS 기본 동작 수행 (`SIGSEGV`는 코어 덤프 후 종료 등)
- 커스텀 핸들러: 응용 프로그램이 `signal()`/`sigaction()` 시스템 호출로 자체 함수 등록
