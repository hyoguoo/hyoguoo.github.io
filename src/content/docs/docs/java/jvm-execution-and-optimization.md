...
tags: [ Java ]
description: "JVM 언어 생태계와 바이트코드 실행 메커니즘, JIT 계층 컴파일 최적화, GraalVM Native Image AOT 컴파일과 그 트레이드오프를 분석한다."
---
...
JIT는 단순히 바이트코드를 기계어로 1:1 번역하는 것이 아니라, 런타임 프로파일을 기반으로 추측을 깔고 정적 컴파일러가 할 수 없는 공격적 최적화를 수행한다.
...
- Spring Boot 3.x: 별도의 Spring Native 인큐베이터 프로젝트는 종료되고, Spring Framework 6 / Spring Boot 3.0부터 GraalVM 네이티브 이미지 지원이
  Spring AOT 형태로 본 프레임워크에 통합
