...
3. Age 증가: 복사된 객체는 객체 헤더의 age 필드가 1 증가하며 생존 횟수가 누적됨
4. Old 영역 승격(Promotion): age가 Tenuring Threshold를 넘기거나 To Survivor 공간이 부족한 경우 Old 영역으로 이동

#### From/To Space 구조

두 Survivor 영역은 항상 한쪽이 비어있는 To Space, 다른 한쪽이 살아있는 객체를 담은 From Space로 역할을 번갈아 맡는다.
...
객체가 Survivor 사이를 이동할 때마다 누적되는 age 값으로 Old 영역 승격 시점을 결정한다.
...
- Adaptive Tenuring: JVM은 To Survivor 점유율이 `TargetSurvivorRatio`(기본 50%)를 넘지 않도록 임계치를 동적으로 낮춰 조기 승격을 유도
- Premature Promotion: To Survivor 공간 부족 시 age와 무관하게 즉시 Old로 승격되며, 잦은 발생은 Old 영역 가비지 누적과 Major GC 빈도 증가로 이어짐
