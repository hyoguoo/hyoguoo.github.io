...
### Weighted Average per Endpoint Class

엔드포인트마다 처리 비용이 다르므로, 단일 평균 응답 시간으로 시스템 천장을 추정하면 결과가 왜곡되기 때문에 가중치를 두고 계산해야한다.

1. 부류별 호출 비중을 가중치로 두고 계산
2. 응답 시간(R)과 DB 점유 시간(D)의 가중 평균을 산출

```text
R̄ = Σ(weight_i × R_i)    (전체 응답 가중 평균, 스레드 천장 산출용)
D̄ = Σ(weight_i × D_i)    (DB 점유 가중 평균, 풀 천장 산출용)
```

- 분리하는 이유: 스레드 천장(`threads / R̄`)과 풀 천장(`pool / D̄`)이 따로 결정됨
- CPU-bound 엔드포인트나 외부 호출 엔드포인트는 R은 크지만 D는 작아, 단일 평균으로는 풀 천장을 과대평가하기 쉬움
- 부류 분류 기준: 단순 조회 / 조인·페이징 / 네트워크 호출 / CPU 집약(인증·암호화) / 트랜잭션 쓰기 등 처리 비용 중심으로 묶음
