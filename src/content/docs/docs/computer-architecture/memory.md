...
메모리에 데이터가 어떻게 적재되는지에 따라 CPU 캐시의 공간 지역성 활용도가 달라진다.

- 연속 할당 (예: 배열): 데이터가 물리적 메모리에 연속해서 존재하므로, 첫 번째 요소를 읽어올 때 다음 요소들도 함께 캐시에 적재
    - 이로 인해 순회 탐색 시 캐시 적중률 (Hit Rate)이 극대화됨.
- 비연속 할당 (예: 연결 리스트): 각 노드가 메모리 여기저기에 산발적으로 흩어져 할당되고 포인터로 연결
    - 다음 노드 주소를 찾아갈 때마다 캐시에 없을 확률이 높아 빈번한 캐시 미스 (Cache Miss)와 메인 메모리 접근 지연이 발생
