...
## DI와 인터페이스 (역할과 구현의 분리)

의존성 주입은 주입받는 타입을 구체 클래스 (Concrete Class)가 아닌 인터페이스 (Interface)로 선언할 때 더 높은 활용도를 가진다.

- 다형성을 통한 느슨한 결합 (Loose Coupling)
    - 클라이언트는 주입받을 객체의 '역할 (인터페이스)'만 알 뿐, 실제 작동하는 '구현체 (구체 클래스)'가 무엇인지 알 필요가 없음
    - 구체 클래스에 직접 의존하면 구현체가 바뀔 때마다 클라이언트 코드도 함께 수정해야 하는 강결합 (Tight Coupling) 문제 발생
- OCP (개방-폐쇄 원칙) 만족 및 런타임 교체 메커니즘
    - 클라이언트 코드의 수정 없이 (Closed), 외부 빈 설정만 변경하여 새로운 구현체로 유연하게 확장 (Open) 가능함
    - 예: 로컬 테스트 시 `DummyPayment`, 운영 시 `TossPayment`를 주입하도록 설정하면 비즈니스 로직 수정 없이 환경별 대처가 가능함
- 전략 패턴 (Strategy Pattern)의 완성
    - 런타임에 외부 (스프링 컨테이너)가 알맞은 전략 (구현체)을 클라이언트에게 꽂아주는 구조로 작동
