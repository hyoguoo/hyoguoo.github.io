...
```mermaid
flowchart TD
    New((New)) -->|persist| Managed((Managed))
...
OSIV는 영속성 컨텍스트의 생명주기를 트랜잭션 범위 (Service 계층)를 넘어 클라이언트 응답 (Controller 계층) 시점까지 확장하는 기능이다.
...
- 실무적 대안: 대량의 트래픽을 다루는 환경에서는 반드시 OSIV 설정을 `false`로 끄고 사용
    - 지연 로딩이 필요한 데이터는 트랜잭션이 끝나기 전에 페치 조인 (Fetch Join)이나 DTO 변환을 통해 미리 가져오도록 설계
