...
tags: [ Spring ]
description: "Spring MVC의 핵심인 DispatcherServlet의 요청 처리 흐름과 doDispatch() 내부 동작을 핸들러 매핑부터 뷰 렌더링까지 분석한다."
---
...
sequenceDiagram
    ...
    participant HM as 핸들러 매핑
    participant HA as 핸들러 어댑터
    participant H as 핸들러(컨트롤러)
    participant VR as viewResolver
    participant V as View
    C ->> DS: 1. HTTP 요청
    DS ->> HM: 2. 핸들러 조회
    HM -->> DS: 핸들러
    DS ->> HA: 3. handle(handler)
    HA ->> H: 4. handler 호출
    H -->> HA: ModelAndView
    HA -->> DS: 5. ModelAndView 반환
    DS ->> VR: 6. viewResolver 호출
    VR -->> DS: View 반환
    DS ->> V: 7. render(model)
    V -->> C: HTML 응답
```
