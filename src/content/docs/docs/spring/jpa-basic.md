...
데이터베이스 연결 구조에서 병목을 제거하고 시스템을 보호한다.

- 연결 오버헤드 제거: 미리 맺어둔 커넥션을 재사용하여 무거운 연결 과정 생략 및 응답 지연 단축
- 데이터베이스 보호: 풀 크기를 제한하여 트래픽 폭증 시에도 DB 서버가 다운되지 않도록 방어
- 프레임워크 호환성: 락 경쟁을 최소화한 설계로 Spring Boot 기본 데이터소스로 채택

### 커넥션 풀 미사용 시 발생하는 문제

매 요청마다 커넥션을 맺고 끊으면 데이터베이스에 막대한 부하를 유발한다.

- 막대한 연결 비용: 쿼리 실행 전마다 TCP 3-Way Handshake와 인증을 거쳐 심각한 지연 발생
- DB 서버 셧다운: `max_connections` 한계를 초과한 무수한 연결 시도가 DB의 CPU와 메모리 고갈 초래
- 임시 포트 고갈: 짧은 연결 반복으로 닫힌 소켓들이 `TIME_WAIT` 상태로 대기하며 OS 포트 소진
