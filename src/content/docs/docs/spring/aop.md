...
- 위빙 (Weaving)
    ...
    - 위빙을 통해 핵심 기능 코드의 변경 없이 부가 기능을 추가할 수 있음
- AOP 프록시 (AOP Proxy)
    - AOP 기능을 구현하기 위해 만든 프록시 객체

## JDK 동적 프록시 / CGLIB

런타임에 프록시 객체를 동적으로 생성하기 위해 스프링이 지원하는 두 가지 방식이 존재한다.

### JDK 동적 프록시

- 자바 표준 라이브러리 (`java.lang.reflect.Proxy`)가 제공하는 기술
- 인터페이스 기반으로 프록시를 생성하므로, 타겟이 반드시 하나 이상의 인터페이스를 구현 필요
- `InvocationHandler` 인터페이스를 구현하여 프록시의 동작 (어드바이스)을 정의
...
- 외부 라이브러리로, 바이트코드를 조작하여 프록시를 동적으로 생성
- 클래스 상속 기반으로 프록시를 생성하므로 인터페이스가 필요 없음
- 타겟 클래스를 상속받아 자식 클래스를 만들기 때문에, 클래스나 메서드에 `final` 키워드가 있으면 오버라이딩이 불가능해 적용할 수 없음
...
Proxy Factory는 대상 객체에 인터페이스 유무를 판단하여 자동으로 적절한 프록시 기술 (JDK or CGLIB)을 선택해 생성해 주는 팩토리 클래스다.
...
- 부가 기능이 필요한 대상 (Target)마다 설정 코드에서 `ProxyFactory`를 직접 호출해 빈으로 등록해야 함
- 컴포넌트 스캔 (`@Component`)을 통해 자동 등록되는 빈에는 수동으로 프록시 객체를 끼워 넣을 방법이 없음

때문에 컴포넌트 스캔을 포함한 등록되는 빈들에 대해 프록시 객체를 적용하는 빈 후처리기 방법이 등장하게 되었다.

## 빈 후처리기 (BeanPostProcessor)

빈 후처리기를 이용하면 컴포넌트 스캔으로 빈이 컨테이너에 등록되기 직전에 원본 객체를 프록시 객체로 바꿔치기 (조작)할 수 있다.
...
하지만 이 방법을 사용하더라도 어드바이저 (포인트컷+어드바이스)를 직접 코드로 생성하고 후처리기를 직접 등록해야 하므로 여전히 관리 비용이 높다는 단점이 존재한다.

## @Aspect 애노테이션

스프링 프레임워크가 제공하는 `@Aspect` 애노테이션 기반의 AOP는 가장 선언적이고 간결하게 프록시 기반 부가 기능을 적용할 수 있는 방식이다.
...
- 프록시 객체 적용
    1. 스프링 빈 대상이 되는 원본 객체 생성
    2. 빈 후처리기가 동작하여 빈을 컨테이너에 등록하기 전 가로챔
    3. 컨테이너에서 모든 어드바이저를 조회하여 현재 생성된 빈이 포인트컷 (적용 대상)에 부합하는지 검사
    4. 대상이라면 프록시 팩토리를 통해 해당 빈의 프록시 객체를 동적 생성함
    5. 원본 객체 대신 프록시 객체를 빈으로 등록 (실제 실행 시 프록시가 먼저 호출)
