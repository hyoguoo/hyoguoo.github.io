...
데이터 전송 시 실제 내용 (Payload) 외에 각 계층의 제어 정보가 헤더로 붙는다.
...
작은 데이터를 보내게 된다면, 데이터보다 헤더가 더 커지는 헤더 오버헤드가 발생할 수 있다.
...
1. 서버 준비
    - 서버는 소켓을 생성하고 특정 포트 (예: 80)에 바인딩 (Bind)
    - 클라이언트의 연결을 기다리는 리슨 (Listen) 상태로 대기
2. 클라이언트 연결
    - 클라이언트는 서버의 IP와 포트로 연결 (Connect) 시도
    - 이 과정에서 TCP 3-way Handshake 발생
3. 데이터 송수신
    - 연결 수립 후 클라이언트는 요청 데이터를 소켓에 씀 (Write)
    - OS는 이를 패킷으로 만들어 전송하고, 서버는 소켓에서 데이터를 읽고 (Read) 처리
4. 연결 종료
    - 데이터 교환이 끝나면 소켓을 닫음 (Close)
    - TCP 4-way Handshake를 통해 리소스 정리
    - HTTP/1.1 Keep-Alive의 경우 연결을 유지하여 여러 요청 처리 가능
