...
### SSL Handshake (TLS 1.2 기준)

TCP 3-way Handshake로 논리적 연결이 맺어진 직후, 보안 통신을 위한 TLS Handshake가 추가로 수행된다.
...
최신 TLS 1.3은 보안이 취약한 알고리즘을 제거하고, 위 핸드셰이크 과정을 간소화하여 (1-RTT) 연결 속도를 개선했다.

- 클라이언트가 첫 요청 (`Client Hello`)을 보낼 때 암호화 방식과 키 교환 데이터를 한 번에 묶어서 서버로 전송
- 불필요한 왕복 횟수가 1-RTT (Round Trip Time)로 줄어듬

## 사이트 인증서 검사

SSL 자체는 사용자에게 인증서 검증 과정을 하지 않지만, 최신 웹 브라우저들은 백그라운드에서 신뢰성을 엄격하게 검증한다.

### 인증서 검사 절차

브라우저는 통신을 수립하기 전 다음과 같은 단계로 서버 인증서를 검증한다.
