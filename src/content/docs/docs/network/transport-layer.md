...
최소 20바이트에 달하는 TCP 헤더와 달리, UDP 헤더는 단 8바이트로 구성되어 있어 오버헤드가 작다.
...
- Source/Destination Port: 송수신 프로세스 식별을 위한 포트 번호
- Length: 헤더와 데이터를 포함한 UDP 데이터그램의 전체 길이
- Checksum: 최소한의 오류 검출을 위한 값

#### TCP와의 트레이드오프 및 활용 사례

UDP는 신뢰성을 포기한 대신 3-Way Handshake 대기 시간과 혼잡 제어 등의 오버헤드를 제거하여 속도를 극대화했다.

- 비연결형 및 비신뢰성
    - 핸드셰이크 과정 없이 데이터를 즉시 전송
    - 수신 여부 확인 (ACK)이나 순서 보장, 흐름 제어를 하지 않음
- 적용 사례별 채택 이유
    - DNS: 요청과 응답 패킷 크기가 매우 작아, 연결 수립 (3-Way Handshake)에 드는 비용이 실제 데이터 전송 비용을 초과하는 상황 방지
    - 실시간 스트리밍 및 영상 통화: 패킷이 일부 유실되어 화질이 열화되더라도, TCP처럼 재전송을 기다리며 전체 화면이 멈추는 (버퍼링) 현상을 막는 것이 중요
    - HTTP/3 (QUIC): TCP의 구조적 지연 (HOL Blocking)을 피하고자 가벼운 UDP 기반 위에 독자적인 신뢰성 제어를 애플리케이션 수준에서 구현
...
데이터 전송이 끝난 후 리소스를 정리하는 과정이다.

- 양방향 통신이므로 송/수신 연결을 각각 독립적으로 닫음
- 연결 수립 (3-Way)과 달리 4단계가 필요한 이유
    - 클라이언트가 종료 요청 (FIN)을 하더라도 서버 측에서는 아직 전송하지 못한 남은 데이터가 있을 수 있음
    - 따라서 서버는 먼저 확인 응답 (ACK)을 전송 (`CLOSE_WAIT` 상태)
    - 남은 데이터를 모두 전송한 뒤에 자신의 종료 요청 (FIN)을 따로 전송
