...
로드밸런서가 어느 계층의 정보를 보고 분산하느냐에 따라 나뉜다.
...
## 하드웨어 vs 소프트웨어 vs 클라우드 로드밸런서

로드밸런서는 물리적 장비에서 소프트웨어, 그리고 클라우드 관리형 서비스로 변화했다. 

|   구분    |                                     하드웨어 (L4/L7 스위치)                                      |                                           소프트웨어 (웹 서버/프록시)                                            |                                  클라우드 매니지드                                  |
|:---------:|:------------------------------------------------------------------------------------------------:|:----------------------------------------------------------------------------------------------------------------:|:-----------------------------------------------------------------------------------:|
| 대표 예시 |                                   F5 BIG-IP, Citrix ADC, Cisco                                   |                                              Nginx, HAProxy, Envoy                                               |                          AWS ELB (ALB, NLB), GCP Cloud LB                           |
| 패킷 처리 |                        전용 칩(ASIC/FPGA)을 통한 하드웨어 레벨 직접 처리                         |                      범용 OS(Linux) 커널의 네트워크 스택을 거쳐 애플리케이션 레벨에서 처리                       |           클라우드 벤더가 프로비저닝한 가상 프록시 클러스터에서 분산 처리           |
|   장점    | 커널 오버헤드가 없어 초당 수백만 건의 패킷 처리 가능<br>물리 계층 수준의 방어(DDoS 등) 성능 보장 | 오픈소스 기반으로 도입 비용 저렴<br>유연한 라우팅 설정 및 커스텀 로직 확장<br>온프레미스·클라우드 환경 제약 없음 | 인프라 관리 부담 없음 (이중화 구성 불필요)<br>트래픽 변화에 따른 자동 스케일링 지원 |
|   단점    |           장비 도입 및 유지보수 비용 고가<br>트래픽 급증 시 기민한 물리 장비 증설 한계           |          OS 컨텍스트 스위칭 및 소켓 버퍼 복사 오버헤드 발생<br>고가용성(이중화) 구성을 직접 제어해야 함          |      클라우드 벤더 종속성 발생<br>커스텀 라우팅이나 세밀한 타임아웃 튜닝 제한       |

### 아키텍처 선택 기준

인프라 환경 변화에 따라 로드밸런서 채택 기준이 달라진다.

- 과거 (온프레미스): 커널 병목 없이 와이어 스피드 (Wire-speed) 처리가 가능한 하드웨어 로드밸런서 필수 도입
- 과도기 (MSA 도입): 컨테이너 환경의 유연성을 위해 소프트웨어 로드밸런서 비중 증가 (OS 네트워크 스택 오버헤드는 DPDK/eBPF 등으로 완화)
- 현재 (클라우드 네이티브): 분산 클러스터링과 자동 확장을 내장한 클라우드 매니지드 로드밸런서 기본 채택
- 예외 상황: 클라우드 간 이식성 보장이나 커스텀 라우팅 로직이 필요한 영역에 한해 소프트웨어 프록시 병행 구성
