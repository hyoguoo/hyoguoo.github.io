...
|:-----------:|:--------------------------------------:|:----------------------------------------:|:---------------------------------------------------:|
|    Dummy    |   전달되지만 실제로는 사용되지 않는 객체(파라미터 자리 채움)    | 시그니처상 인스턴스가 필요하지만 해당 테스트 경로에서 호출되지 않는 경우 |         검증 경로에서 호출되지 않는 Logger 등 의존성 자리 채움          |
...
public void dummyTest() {
    // MyService 생성자가 Logger를 요구하지만, 아래 검증 경로에서는 로깅이 일어나지 않음
    DummyLogger dummyLogger = new DummyLogger();
    MyService service = new MyService(dummyLogger);
    service.calculateTotal(); // dummyLogger는 자리만 채울 뿐, log()는 호출되지 않음
}
```
