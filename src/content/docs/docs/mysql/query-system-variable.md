...
tags: [MySQL]
description: "MySQL sql_mode 시스템 변수와 문자셋·정렬 규칙 설정이 SQL 구문 지원 범위와 데이터 유효성 검사에 미치는 영향을 설명한다."
---
...
데이터베이스, 테이블, 컬럼 등 식별자의 이름을 MySQL이 내부적으로 사용하는 예약어(예: `SELECT`, `ORDER`)를 키워드로 생성하기 위해서 백틱(`)으로 감싸서 사용할 수 있다.
