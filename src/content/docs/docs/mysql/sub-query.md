...
### 구체화 (Materialization) 전략의 한계와 옵티마이저 튜닝

옵티마이저가 파생 테이블 (임시 테이블)을 생성하여 구체화하는 전략은 대용량 데이터 조회 시 심각한 병목을 유발할 수 있다.

- 인덱스 부재: 동적으로 생성된 파생 테이블에는 인덱스가 존재하지 않으므로, 외부 테이블과 조인 시 풀 테이블 스캔 (All)을 피할 수 없음
- 튜닝 전략: 대량의 통계 데이터를 다룰 때 `GROUP BY`가 포함된 서브쿼리에서 지연이 발생한다면, 구체화를 유발하는 서브쿼리 구조를 해체 필요
- 인덱스 레인지 스캔 (Range Scan) 유도: 기준이 되는 테이블과 명시적인 조인 (JOIN)을 맺어 인덱스를 먼저 타게 필터링한 후 그룹핑을 수행하도록 쿼리를 재설계
