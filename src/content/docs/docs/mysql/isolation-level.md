...
- 언두 로그 팽창(Undo Log Bloat): 참조할 가능성이 있는 언두 로그를 삭제하지 못하고 계속 보관해야하기 때문에 대량 작업이나 긴 트랜잭션 주의 필요
    - 언두 테이블스페이스의 급격한 크기 증가와 시스템 성능 저하를 유발 가능
...
하지만 InnoDB 스토리지 엔진에서는 `REPEATABLE READ` 격리 수준에서도 `PHANTOM READ` 문제를 해결할 수 있어 해당 격리 수준을 사용하지 않는다.
