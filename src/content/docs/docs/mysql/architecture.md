...
InnoDB는 디스크 I/O를 최소화하고 데이터 무결성을 보장하기 위해 메모리 캐시(Buffer Pool)와 쓰기 지연 로그(Redo/Undo Log)가 유기적으로 협력하는 구조를 가진다.

```text
 Client SQL
    │
    ▼
[ Buffer Pool ] ← 디스크 페이지의 메모리 캐시 (16KB 페이지 단위)
    │           ├─ Change Buffer: Non-unique 세컨더리 인덱스 변경의 지연 병합
    │           └─ Adaptive Hash Index: 자주 쓰는 키의 해시 조회 최적화
    ▼
[ Redo Log ] ← 변경된 페이지의 "변경 사실"만 먼저 디스크에 순차 기록 (WAL)
    │
    ▼
[ Data File ] ← 실제 테이블·인덱스 (백그라운드에서 느슨하게 flush)

[ Undo Log ] ← 각 트랜잭션의 "이전 값" 보관. MVCC 스냅샷 복원 + 롤백 근거
```

1. Buffer Pool (캐시 및 지연 쓰기): 모든 데이터 변경은 메모리에서 먼저 일어나며, 디스크 I/O 부하를 줄이기 위한 다양한 최적화 구조 내장
2. Redo Log (영속성 보장): 변경 사항은 손실 방지를 위해 WAL(Write Ahead Log) 원칙에 따라 디스크 리두 로그에 즉시 순차 기록
3. Undo Log (일관성 및 복구): 변경 전 데이터를 별도 보관하여 MVCC와 트랜잭션 롤백 수행
4. Data File (최종 저장): 리두 로그가 안전하게 기록된 후, 체크포인트 메커니즘을 통해 버퍼 풀의 변경 내용이 실제 데이터 파일에 비동기적으로 반영
...
- 언두 체인(Undo Chain): `DB_ROLL_PTR`을 따라가면 해당 레코드의 과거 이력을 순차적으로 탐색 가능
    - InnoDB의 일관된 읽기 로직이 Read View와 `DB_TRX_ID`를 비교하여, 어떤 버전의 데이터를 읽어야 할지 결정
...
- 트랜잭션의 ACID 속성 중 영속성(Durability)을 보장하기 위한 메커니즘으로, WAL(Write Ahead Log) 방식으로 기록
- 장애 복구 메커니즘: 서버 비정상 종료 시 리두 로그와 언두 로그가 협력하여 복구 수행
    - Redo: 커밋되었으나 아직 데이터 파일에 반영되지 못한 변경 사항을 재적용(Replay)하여 데이터 복원
    - Undo: 커밋되지 않은 트랜잭션이 데이터 파일에 이미 반영되어 있다면, 이를 롤백하여 데이터의 일관성을 맞춤
...
graph TD
    ...
    BP -- " 1. 변경사항 기록 " --> RLB
    RLB -- " 2. WAL: 커밋 시 로그 플러시 " --> RL
    BP -. " 3. 체크포인트: 지연된 쓰기 " .-> DF

    subgraph "Crash Recovery"
        direction LR
        Redo["Redo: 커밋된 변경 재적용"]
        Undo["Undo: 완료되지 않은 변경 롤백"]
    end
...
- InnoDB의 핵심 메모리 영역
- 디스크의 데이터 파일과 인덱스 정보를 페이지 단위(기본 16KB)로 메모리에 캐시
- 데이터 읽기: 디스크가 아닌 버퍼 풀에서 우선적으로 데이터를 찾아 랜덤 I/O를 최소화
- 데이터 쓰기: 디스크에 즉시 기록하지 않고 버퍼 풀의 페이지만 변경(= Dirty Page)한 뒤, 체크포인트 타이밍에 백그라운드 스레드가 디스크에 점진적으로 기록(Flush)

### 체인지 버퍼(Change Buffer)

- 유니크하지 않은 세컨더리 인덱스의 변경 사항을 임시로 기록하여 디스크 I/O를 지연시키는 공간
- 동작 방식: 변경해야 할 인덱스 페이지가 메모리에 없다면 즉시 읽어오지 않고 체인지 버퍼에 기록
    - 이후 백그라운드 스레드에 의해 실제 인덱스 페이지와 병합
- 제약: 유니크 인덱스는 중복 여부를 즉시 체크해야 하므로 체인지 버퍼를 사용 불가능
