...
tags: [ MySQL ]
description: "B-Tree(B+ Tree) 인덱스의 리프 노드 구조와 인덱스 스캔 방식, 자료형이 트리에 미치는 영향을 분석한다."
---

> B-Tree 인덱스는 데이터베이스의 인덱스를 구성하는 가장 기본적인 방법이다. Binary의 B가 아니라 Balanced의 B를 의미  
> InnoDB를 포함한 대부분의 RDBMS는 일반적인 B-Tree가 아닌, 이를 개선한 B+ Tree 구조를 사용

## 구조 및 특성

### B+ Tree의 특징 (B-Tree와의 차이점)

최상위에 하나의 루트 노드가 존재하고, 중간에 브랜치 노드, 가장 하위에 리프 노드가 존재하는 기본 형태는 동일하지만 다음과 같은 차이가 존재한다.

- 데이터 저장 위치: 일반 B-Tree는 모든 노드 (루트, 브랜치, 리프)에 데이터를 저장하지만 B+ Tree는 오직 리프 노드에만 실제 데이터 (또는 데이터의 위치 식별자)를 저장
    - 브랜치 노드는 자식 노드를 찾기 위한 내비게이션 역할만 수행
- 리프 노드 간의 연결: 리프 노드들은 서로 Linked List (이중 연결 리스트) 형태로 연결

이러한 구조적 차이는 레인지 스캔 (Range Scan)에 압도적인 이점을 제공한다.

- 특정 범위의 데이터를 찾을 때 트리를 매번 루트부터 탐색할 필요 없음
- 리프 노드에 도달한 후 연결 리스트를 따라 순차적으로 읽어서 조회 가능
...
인덱스는 페이지 단위로 관리되기 때문에, 각 노드에서 가질 수 있는 자식 노드의 개수는 인덱스 키 값의 자료형 크기에 따라 결정된다.

- 인덱스 페이지 크기: 16KB
- 인덱스 키: 16 Byte
- 자식노드 주소: 12 Byte (대략적인 평균값)
- 하나의 인덱스 페이지에 저장할 수 있는 최대 키 값의 개수: 16 * 1024 / (16 + 12) = 585개

인덱스의 키가 커지게 되면, 그만큼 하나의 인덱스 페이지에 저장할 수 있는 키 값의 개수가 줄어들게 되면서, 디스크 I/O 횟수가 증가하고 그만큼 성능이 저하된다.

#### 순차 I/O와 페이지 분할 (INT vs UUID)

크기뿐만 아니라 데이터가 삽입되는 패턴 (자료형의 특성)도 매우 중요하다.

- Auto Increment (INT / BIGINT)
    - 값이 순차적으로 증가하므로, 새로운 레코드가 리프 노드의 맨 끝에만 추가
    - B+ 트리 구조가 안정적으로 유지되며 추가 비용이 적음
- UUID (VARCHAR / BINARY)
    - 값이 무작위로 생성되므로, 새로운 레코드가 기존 리프 노드의 중간중간에 삽입됨
    - 리프 노드가 꽉 찬 상태에서 중간에 데이터가 삽입되면 페이지 분할 (Page Split)이 발생
    - 페이지 분할은 상위 브랜치 노드까지 업데이트해야 하므로 막대한 디스크 쓰기 I/O 비용을 발생

### 2. B-Tree 깊이와 디스크 I/O

B-Tree 인덱스의 깊이는 성능을 결정짓는 핵심 요소로, 루트 노드에서 리프 노드까지 내려가는 단계가 곧 디스크 I/O 횟수를 의미한다.
