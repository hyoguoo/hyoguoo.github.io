...
tags: [ MySQL ]
description: "MySQL 파티셔닝의 동작 원리와 Range·Hash 파티셔닝의 트레이드오프, 그리고 샤딩과의 차이점을 면접 및 실무 관점에서 분석한다."
---
...
대용량 데이터로 인한 성능 저하 시나리오에서는 분산 비용과 복잡도를 고려해 다음 순서로 아키텍처를 확장한다.

1. 쿼리 및 인덱스 튜닝
    - 슬로우 쿼리 최적화 및 커버링 인덱스 적용
2. 스케일 업 (Scale-up) 및 읽기 분산
    - 하드웨어 사양 증가 및 레플리케이션 (Replication)을 통한 읽기 트래픽 분산
3. 파티셔닝 (Partitioning)
    - 단일 테이블 크기 증가로 인덱스 트리가 깊어지고 데이터 백업·삭제 관리가 어려워질 때 도입
4. 샤딩 (Sharding)
    - 쓰기 트래픽 처리량이 단일 DB 인스턴스의 물리적 한계 (CPU, 디스크 I/O)를 초과할 때 도입

샤딩 도입 시 애플리케이션 레벨에서 해결해야 할 분산 시스템의 제약이 발생한다.

- Cross-shard Join 불가: 서로 다른 샤드 간 데이터는 SQL 조인이 불가능해 애플리케이션 로직에서 조합 필요
- 분산 트랜잭션 복잡성: 여러 샤드에 걸친 트랜잭션 처리 시 2PC, Saga 패턴 등을 도입해야 하므로 정합성 보장 난이도 상승
- 데이터 재분배 (Re-sharding) 위험: 샤드 증설 시 대량의 데이터를 다운타임 없이 마이그레이션하기 어려움
