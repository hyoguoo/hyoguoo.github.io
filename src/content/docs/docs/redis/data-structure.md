...
레디스의 각 자료구조는 외부로 보이는 추상 타입 (String·List·Hash 등)은 동일하지만, 내부적으로는 원소 개수와 값 크기에 따라 서로 다른 물리 구현 (인코딩)으로 저장된다.

- 인코딩 전환 이유
    - 원소가 적고 작은 컬렉션은 메모리 지역성이 높은 컴팩트 포맷 (listpack/intset 등)에 저장해 메모리 사용량을 줄이고 CPU 캐시 친화성을 높임
    - 규모가 커지면 O (logN)이나 O (1) 연산을 유지하기 위해 해시 테이블·스킵 리스트 같은 구조로 자동 전환
- 전환 방향: 한 번 큰 인코딩으로 승격되면 원소가 다시 줄어들어도 대부분 되돌아가지 않음 (`CONFIG SET`이나 재생성 필요)
- 현재 인코딩은 `OBJECT ENCODING key`로 확인 가능
...
- 내부 인코딩
    ...
    - `raw`: 45바이트 이상 긴 문자열. 헤더와 데이터를 분리 할당
...
순서를 가지는 문자열 목록으로, 양 끝 삽입·제거와 인덱스 접근을 지원한다.
...
- 내부적으로는 quicklist로 구현 → 여러 개의 listpack 노드를 양방향 연결 리스트로 이은 구조
    - 작은 단위 (listpack)에 모아 메모리 지역성을 유지하면서, 노드 간에는 연결 리스트라 양 끝 O (1) 연산이 가능
    - `list-max-listpack-size`(기본 -2, 노드당 8KB 상한)로 노드 크기 조절
...
- 내부 인코딩
    - listpack: 필드 수와 값 길이가 모두 작을 때 사용 → 포인터 없이 연속 메모리로 저장해 메모리 오버헤드 최소화
    - hashtable: 임계값 초과 시 일반 해시 테이블로 전환 → O (1) 필드 접근 유지
    - 전환 임계값: `hash-max-listpack-entries`, `hash-max-listpack-value`(기본 64바이트) 중 하나라도 초과하면 hashtable로 승격
...
- 내부 인코딩 (삼단 전환)
    - intset: 모든 원소가 정수이고 개수가 `set-max-intset-entries`(기본 512) 이하일 때. 정렬된 배열로 이진 탐색 O (logN), 최소 메모리
    - listpack: 원소 중 하나라도 문자열이고 크기가 작을 때 (Redis 7.2+). `set-max-listpack-entries`(기본 128), `set-max-listpack-value`(기본
      64바이트)
    - hashtable: 위 조건을 벗어나면 해시 테이블로 승격 → O (1) 멤버십 조회 유지
...
- 내부 인코딩
    - listpack: 원소 수와 값 크기가 작을 때. `zset-max-listpack-entries`(기본 128), `zset-max-listpack-value`(기본 64바이트) 이내
    - skiplist + dict (hash table): 임계값 초과 시 사용. 정렬 순회와 범위 조회는 스킵 리스트가, 단일 원소 점수 조회는 해시 테이블이 담당하는 이중 구조 → 범위 조회 O
      (logN), 단건 조회 O (1)
...
##### 이진 탐색 트리 대신 Skip List를 채택한 이유 (트레이드오프)

Sorted Set은 O (logN)의 탐색 성능을 제공하기 위해 일반적인 이진 탐색 트리 (Red-Black Tree, B-Tree 등) 대신 스킵 리스트를 채택했다.

- 범위 검색 (Range Scan) 최적화
    - 트리는 범위 검색 시 상하 이동이 잦은 중위 순회 (In-order traversal) 필요
    - 스킵 리스트는 시작점 탐색 (O (logN)) 후, 모든 데이터가 연결된 최하단 리스트 (Level 0)만 순차 탐색
    - 연속된 메모리 접근으로 캐시 지역성이 높아 범위 조회 (`ZRANGE`)에 유리함
- 균형 유지 (Rebalancing) 오버헤드 최소화
    - 트리는 데이터 삽입/삭제 시 전체 트리의 균형을 맞추기 위한 복잡한 회전 (Rotation) 연산 필요
    - 스킵 리스트는 복잡한 연산 대신 무작위 확률을 기반으로 노드의 층고 (Level)를 결정
    - 상위 레벨로 갈수록 노드 수가 일정 비율로 줄어들어 자연스럽게 균형이 유지되므로 링크 수정만으로 쓰기 연산이 끝남 (오버헤드 작음)
