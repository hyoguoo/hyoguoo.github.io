...
- 인코딩 전환 이유
    ...
    - 규모가 커지면 O(logN)/O(1) 연산을 유지하기 위해 해시 테이블·스킵 리스트 같은 구조로 자동 전환
...
- 내부 인코딩
    ...
    - 전환 임계값: `hash-max-listpack-entries`(기본 128), `hash-max-listpack-value`(기본 64바이트) 중 하나라도 초과하면 hashtable로 승격
...
- 내부 인코딩
    ...
    - skiplist + dict(hash table): 임계값 초과 시 사용. 정렬 순회와 범위 조회는 스킵 리스트가, 단일 원소 점수 조회는 해시 테이블이 담당하는 이중 구조 → 범위 조회 O(logN),
      단건 조회 O(1)
