...
분산된 여러 서버 (노드)들이 하나의 시스템처럼 동작하도록 돕는 "조정자" 또는 "중앙 관리소" 역할을 수행한다.

```mermaid
graph LR
    Z[(ZooKeeper)] <--> C[Controller]
...
### 특정 토픽의 파티션 리더 담당 브로커 장애 발생 시나리오
...
이처럼 카프카는 클러스터의 두뇌 역할을 주키퍼에 맡겨 안정성을 확보하게 된다.
...
- 운영 단순화: 관리할 시스템이 카프카 하나로 줄어 매우 편의성 증가
...
KRaft가 사용하는 Raft 프로토콜은 다음 세 단계로 동작한다.

1. 리더 선출 (Leader Election)
    - 리더가 없거나 기존 리더의 하트비트가 타임아웃되면 팔로워가 후보 (Candidate)로 전환
    - 후보는 자신에게 투표하고 다른 노드에게 투표 요청을 보냄
    - 과반수 이상의 표를 얻은 후보가 새로운 리더로 선출
2. 로그 복제 (Log Replication)
    - 리더만이 새로운 로그 엔트리를 생성하여 자신의 로그에 추가
    - 리더는 이 엔트리를 모든 팔로워에게 전파하고, 과반수 이상의 팔로워가 수신을 확인하면 해당 엔트리를 커밋 (확정)
3. 안전성 보장 (Safety)
    - 로그가 최신이 아닌 후보에게는 투표하지 않아 데이터가 불완전한 노드가 리더가 되는 것을 방지
