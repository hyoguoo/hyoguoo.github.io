...
### 실무적 동기와 문제 정의 (Motivation)

단순히 "이것은 무엇이다"라고 시작하지 않는다.
이 개념을 모를 때 실무에서 어떤 치명적인 문제(예: 메모리 누수, 이벤트 루프 블로킹, 성능 저하 등)가 발생하는지, "왜 이것을 알아야 하는가?"에 대한 동기를 도입부나 결론에 명확히 제시하여 글의 몰입도를 높인다.

### 실전 코드 예시 (Real-world Examples)

이론적 설명이나 추상적인 개념에 머물지 않고, 실무 프레임워크 수준의 안 좋은 코드(Anti-pattern)나 진화 과정을 직접 보여주는 예시 코드를 적극적으로 삽입한다.
이를 통해 독자가 자신이 작성하던 코드와 내부 메커니즘을 직관적으로 연결할 수 있도록 돕는다.
...
기존 문장의 구조나 핵심 서술어를 완전히 지우거나 덮어쓰지 말고, 원본을 최대한 보존한 상태에서 보강할 내용을 '하위 항목'이나 '추가 단락'으로 덧붙인다.
...
수정 범위가 일부라도, 해당 문서 전체가 컨벤션에 맞는지, 새로 추가한 문장에도 톤앤매너가 제대로 적용되었는지 함께 확인한다.

체크리스트:

- [ ] 새로 추가한 문장도 명사형 종결 등 톤앤매너를 준수했음
- [ ] 기존 문장의 뼈대를 불필요하게 파괴하거나 덮어쓰지 않고 자연스럽게 덧붙였음
- [ ] 볼드(`**`) 없음
- [ ] 문체가 `~다.` 또는 명사형
- [ ] 헤더 단위로 의미 구분 (논점이 여러 개면 `###` 소제목 추가)
- [ ] 한 단락에 본문 1문장 + 리스트 (2문장 이상 연속 금지)
- [ ] 검증된 내용만 포함
- [ ] AI체 없음 (빈 강조어·클리셰 비유·중복 마무리·기계적 병렬)
